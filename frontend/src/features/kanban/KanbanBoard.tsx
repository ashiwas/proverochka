...
export function KanbanBoard({
  leads, onOpen, onMove,
}: { leads: Lead[]; onOpen: (id: string) => void; onMove: (leadId: string, status: LeadStatus) => void }) {
    ...
    <DndContext sensors={sensors} onDragEnd={onDragEnd}>
      <div className="flex gap-3 overflow-x-auto pb-4">
        {LEAD_STATUS_ORDER.map((status) => (
          <Column key={status} status={status} leads={leads.filter((l) => l.status === status)} onOpen={onOpen} />
        ))}
      </div>
    </DndContext>
  );
